    ...
    public void setCallback(String theCallback) {
        ...
        if (theCallback.length() > 0) {
            ...
            for (int i = 0; i < propertyList.length; i++) {
                ...
                if (p.length == 2) {
                    if (p[0].equals(ON_START) || p[0].equals(ON_END) || p[0].equals(ON_DELAY_END) || p[0].equals(
                            ON_UPDATE)) {
                        ...
                        if (foundMethod) {
                            ...
                            System.out.println(
                                    ANI_DEBUG_PREFIX + " Error @ AniCore -> setCallbackMethods(). Can't find a method of name: " + targetMethodName);
                        }
                    }
                }
            }
